...
export function SystemOverview({ displayName }: { displayName: string }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[var(--se-page)]">
      <PublicationBanner />
      <header className={`sticky top-0 z-40 flex items-center justify-between gap-3 border-b px-3 py-2 backdrop-blur-xl sm:px-5 ${darkMode ? "border-zinc-800 bg-zinc-950/90" : "border-zinc-200 bg-white/90"}`}>
        ...
        <div className="flex items-center gap-1.5">
          <button aria-label="Search everything" onClick={() => setSearchOpen(true)} className={`rounded-lg px-2.5 py-2 text-xs ${darkMode ? "bg-zinc-900 text-zinc-300 hover:bg-zinc-800" : "bg-zinc-100 text-zinc-700 hover:bg-zinc-200"}`}><span aria-hidden>⌕</span><span className="hidden sm:inline"> Search everything</span></button>
          <button onClick={() => setTrustOpen(true)} className={`hidden rounded-lg px-2.5 py-2 text-xs md:block ${darkMode ? "bg-zinc-900 text-zinc-300" : "bg-zinc-100 text-zinc-700"}`}>{orientation.trust.source_coverage.percent == null ? "Scope" : `${orientation.trust.source_coverage.percent}% mapped`}</button>
          <ThemeSwitcher />
          <button onClick={() => setPreferencesOpen(true)} className={`rounded-lg p-2 text-sm ${darkMode ? "text-zinc-400 hover:bg-zinc-900" : "text-zinc-600 hover:bg-zinc-100"}`} aria-label="Viewer preferences">◒</button>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-[1600px] flex-1 px-4 py-5 sm:px-6 lg:px-10">
        <div className="flex flex-col justify-between gap-4 border-b border-zinc-500/20 pb-4 lg:flex-row lg:items-end">
          ...
          <div className={`grid grid-cols-3 rounded-xl border p-1 ${darkMode ? "border-zinc-800 bg-zinc-900/80" : "border-zinc-200 bg-zinc-100"}`}>
            {(["portrait", "questions", "atlas"] as const).map((item, index) => <button key={item} aria-label={item} onClick={() => setDirection(item)} className={`rounded-lg px-3 py-2 text-[11px] font-semibold capitalize ${direction === item ? darkMode ? "bg-zinc-700 text-white" : "bg-white text-zinc-900 shadow" : "text-zinc-500"}`}><span className="hidden md:inline">0{index + 1} · </span>{item}</button>)}
          </div>
        </div>
...
function Portrait({ orientation, darkMode, onTarget, onComponent, onTrust }: { orientation: OrientationProjection; darkMode: boolean; onTarget: (target: OrientationTarget, question?: string) => void; onComponent: (id?: string) => void; onTrust: () => void }) {
  ...
  return <div className="grid gap-8 py-8 xl:grid-cols-[minmax(0,0.88fr)_minmax(560px,1.12fr)] xl:items-start">
    ...
    <section data-se="panel" className={`relative min-h-[460px] overflow-hidden rounded-[2rem] border p-5 sm:p-8 ${darkMode ? "border-zinc-800 bg-zinc-950/75" : "border-zinc-200 bg-white"}`}>
      <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-wider text-zinc-500">System portrait</p><h3 className={`mt-1 text-xl font-bold ${darkMode ? "text-zinc-100" : "text-zinc-900"}`}>{orientation.portrait.nodes.length} meaningful areas before {architecture.stats.total_components} components</h3></div><button onClick={() => onTarget({ lens: "structure", semantic_level: "system" })} className="text-xs font-semibold text-cyan-500">Full map →</button></div>
      ...
      <button onClick={onTrust} className={`mt-2 text-[10px] ${darkMode ? "text-zinc-500" : "text-zinc-500"}`}>Derived by {orientation.portrait.method}. How do we know? →</button>
    </section>
  </div>;
}
