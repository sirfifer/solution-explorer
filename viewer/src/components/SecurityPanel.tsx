  ...
  return (
    <section data-se="panel" className={container} aria-label="Security lens">
      <header className={`shrink-0 border-b px-4 py-3 ${darkMode ? "border-zinc-800" : "border-zinc-200"}`}>
        <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-500">Repository-observable only</p>
        <h2 className={`mt-1 text-sm font-bold ${darkMode ? "text-zinc-100" : "text-zinc-900"}`}>Security mechanisms and unknowns</h2>
        <p className={`mt-1 text-[10px] leading-relaxed ${darkMode ? "text-zinc-500" : "text-zinc-500"}`}>{security.method_caveat}</p>
      </header>
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
        ...
        <section className={`rounded-xl border p-3 ${darkMode ? "border-amber-500/20 bg-amber-500/5" : "border-amber-200 bg-amber-50"}`}>
          <h3 className={`text-[10px] font-semibold uppercase tracking-wider ${darkMode ? "text-amber-300" : "text-amber-700"}`}>Not observable from this repository</h3>
          <ul className={`mt-2 list-disc space-y-1 pl-4 text-[10px] ${darkMode ? "text-zinc-400" : "text-zinc-600"}`}>
...
function SecurityGroup({ title, darkMode, children }: { title: string; darkMode: boolean; children: ReactNode }) {
  return <section><h3 className={`mb-1 text-[10px] font-semibold uppercase tracking-wider ${darkMode ? "text-zinc-500" : "text-zinc-500"}`}>{title}</h3><div className="space-y-1">{children}</div></section>;
}

function rowClass(darkMode: boolean): string {
  return `block w-full rounded-lg px-2.5 py-2 text-left ${darkMode ? "bg-zinc-900/70 hover:bg-zinc-900 text-zinc-200" : "bg-white hover:bg-zinc-100 text-zinc-800"} [&>strong]:block [&>strong]:text-xs [&>small]:mt-0.5 [&>small]:block [&>small]:text-[10px] [&>small]:text-zinc-500`;
}
