...
const INTERFACES = [
  ...
  },
  {
    ...
    label: "Workbench",
    description: "The full technical workspace for direct, detailed exploration.",
  },
];
  ...
  return (
    <div data-testid="preferences-drawer" className="fixed inset-0 z-[70] flex justify-end bg-black/45" role="dialog" aria-modal="true" aria-label="Viewer preferences">
      ...
      <aside className={`relative h-full w-full max-w-md overflow-y-auto border-l p-6 shadow-2xl ${dark ? "border-zinc-800 bg-zinc-950" : "border-zinc-200 bg-zinc-50"}`}>
        ...
        <section className="mt-6" aria-labelledby="interface-heading">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className={`mt-1 text-xs leading-relaxed ${dark ? "text-zinc-400" : "text-zinc-600"}`}>Move between guided orientation and the full technical workspace. Both are current views of the same system.</p>
            </div>
          ...
          <p className={`mt-3 text-xs leading-relaxed ${dark ? "text-zinc-500" : "text-zinc-600"}`}>
            Viewing <strong className={dark ? "text-zinc-300" : "text-zinc-700"}>{state.architecture?.name ?? "the current projection"}</strong>. Only the <code>mode</code> parameter changes; the data URL and navigation parameters stay the same.
          </p>
        </section>
        <PreferenceSelect label="Start view" value={state.startView} options={[{ value: "overview", label: "Overview" }, { value: "workbench", label: "Workbench" }, { value: "last", label: "Last used" }]} onChange={(value) => state.setStartView(value as typeof state.startView)} dark={dark} />
        ...
        <label className={`mt-5 flex min-h-11 items-center justify-between rounded-xl border p-4 ${dark ? "border-zinc-800 bg-zinc-900" : "border-zinc-200 bg-white"}`}><span><strong className={`block text-sm ${dark ? "text-zinc-200" : "text-zinc-800"}`}>Remember navigation</strong><small className={dark ? "text-zinc-400" : "text-zinc-600"}>Resume the last aperture when Start view is Last used.</small></span><input className="h-6 w-6" type="checkbox" checked={state.rememberNavigation} onChange={(event) => state.setRememberNavigation(event.target.checked)} /></label>
