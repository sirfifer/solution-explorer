...
export function getAnalyzerPath() {
    ...
    throw new Error("Could not find the analyzer. If running from source, make sure you're in the solution-explorer repo.");
}
...
export async function runAnalyzer(python, options) {
    ...
    console.log(pc.dim(`  Running analyzer on ${options.config || options.repoPath}...`));
    try {
        ...
        const error = err;
        throw new Error(`Analyzer failed: ${error.stderr || error.message}`);
    }
}
