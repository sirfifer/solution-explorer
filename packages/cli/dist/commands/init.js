...
import { architectureWorkflow, } from "../templates/architecture.yml.js";
import { liveMonitorWorkflow, } from "../templates/live-monitor.yml.js";
...
export async function init(options) {
    ...
    if (!existsSync(".git")) {
        console.error(pc.red("This directory is not a git repository."), "Run this from your project root.");
        process.exit(1);
    }
    // Gather repo info
        ...
        },
        {
            ...
            choices: [
                ...
                },
                {
                    ...
                    value: "artifact-only",
                    description: "Download from GitHub Actions. No hosting setup needed.",
                },
            ],
        },
        {
            ...
            choices: [
                {
                    title: "GitHub Pages",
                    value: "github",
                    description: "Free. Live data served from GitHub Pages with ~10 min cache.",
                },
                {
                    title: "Cloudflare (enhanced)",
                    value: "cloudflare",
                    description: "Real-time updates via Worker + R2. Requires Cloudflare infrastructure.",
                },
            ],
        });
    ...
    if (options.live) {
        ...
        if (hasGhCli()) {
            ...
            try {
                execSync(`gh api repos/${repoInfo.owner}/${repoInfo.repoName}/pages -X POST --input - <<< '{"build_type":"workflow"}' 2>/dev/null`, { stdio: "pipe" });
                console.log(pc.green("  GitHub Pages enabled."));
            }
            catch {
                console.log(pc.yellow("  Could not auto-enable GitHub Pages."), "Enable manually: Settings > Pages > Source: GitHub Actions");
            }
        }
    }
    ...
    if (answers.deployTo === "cloudflare") {
        ...
        console.log(`  1. Set GitHub secrets: ${pc.cyan("CLOUDFLARE_API_TOKEN")} and ${pc.cyan("CLOUDFLARE_ACCOUNT_ID")}`);
        ...
        console.log(pc.bold("Next steps:"));
        console.log("  1. Enable GitHub Pages: Settings > Pages > Source: GitHub Actions");
        console.log("  2. Commit and push these files");
        console.log("  3. The workflow will run automatically");
        console.log();
    }
        ...
        console.log("  2. Download the visualization from the Actions artifact");
        console.log();
    }
    if (options.live && liveMode === "cloudflare") {
        ...
        console.log(`    ${pc.cyan("CF_R2_ACCESS_KEY_ID")}    R2 access key`);
        console.log(`    ${pc.cyan("CF_R2_SECRET_ACCESS_KEY")} R2 secret key`);
        console.log(`    ${pc.cyan("CF_R2_ENDPOINT")}         R2 endpoint URL`);
        console.log();
        console.log(pc.dim("Deploy the Worker first if you haven't:"));
        console.log(pc.dim("  git clone https://github.com/sirfifer/solution-explorer.git"));
        console.log(pc.dim("  cd solution-explorer/infrastructure/cloudflare && bash setup.sh"));
        console.log();
    }
...
function getRepoInfo() {
    ...
    try {
        const output = execSync("gh repo view --json nameWithOwner,description 2>/dev/null", { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
...
function copyScripts() {
    ...
    if (!scriptsDir) {
        console.log(pc.yellow("  Could not find bundled scripts."), "Copy them manually from the solution-explorer repo.");
        return;
    }
