export function architectureWorkflow(options) {
    const branchList = options.branches.map((b) => `'${b}'`).join(", ");
    const cloudflareInputs = options.deployTo === "cloudflare"
        ? `
        ...
        : options.deployTo === "github-pages"
          ...
          deploy-to: github-pages`
            : "";
    const permissions = options.deployTo === "github-pages"
        ? `
permissions:
  contents: read
  pages: write
...
permissions:
  contents: read`;
    const environment = options.deployTo === "github-pages"
        ? `
    environment:
      name: github-pages
      url: \${{ steps.deploy.outputs.page_url }}`
        : "";
    const deployStep = options.deployTo === "github-pages"
        ? `
