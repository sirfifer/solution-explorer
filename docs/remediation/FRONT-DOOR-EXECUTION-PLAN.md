...
Current interface policy: Overview is the default guided SysCorpus interface and Workbench is the current full technical interface. Both are durable apertures over the same projection and route state. This file records their implementation path; it is not a second active product plan.
