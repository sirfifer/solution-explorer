...
| 2026-07-13 | P4-3 assembly | Project git info (repository URL from `.git/config`, default branch from `.git/HEAD`) and Info.plist app names are not store facts, so derivation emits repository None, default_branch "main", and directory-based names for Info.plist apps. Fixtures are unaffected (no `.git`, no plists). | P4-5 projection should source git info at projection time (it is environment, not code, so reading it there does not violate the derive-reads-only-the-store rule) and decide whether plists join the ci_config-style rescue. |
