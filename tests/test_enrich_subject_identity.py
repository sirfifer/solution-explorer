...
VSCODE_BAD_SUMMARY = (
    "This is a fork of code-oss-dev (VS Code's open-source codebase) heavily "
    "extended with AI capabilities. It is distinguished from upstream VS Code."
)
...
def test_canonical_snapshot_is_the_fail_closed_default():
    ...
    assert identity["repository"] == "https://github.com/microsoft/vscode"
...
def test_vscode_failure_is_rejected_but_a_vendored_child_fork_is_not():
    identity = build_subject_identity({"name": "vscode"})

    assert subject_identity_errors({"summary": VSCODE_BAD_SUMMARY}, identity)
    assert not subject_identity_errors({
        "summary": (
            "Visual Studio Code contains a vendored fork of an autocomplete "
...
def test_architecture_prompt_receives_repository_and_snapshot_semantics():
    facts = SimpleNamespace(arch={
        "name": "vscode", "components": [], "relationships": [], "stats": {},
    })
    identity = build_subject_identity({
        "name": "vscode",
        "repository": "https://github.com/microsoft/vscode",
    ...
    assert "https://github.com/microsoft/vscode" in prompt
...
def test_orientation_and_universal_release_bar_share_the_identity_contract():
    ...
    prompt = build_orientation_prompt(
        name="vscode", description="", stats={}, readme="",
...
def test_final_review_sees_the_actual_narrative_and_identity_contract():
    prompt = build_determination_prompt(
        ...
        architecture_narrative={"summary": VSCODE_BAD_SUMMARY},
    )

    assert "THE ACTUAL PUBLISHED ARCHITECTURE NARRATIVE YOU MUST REVIEW" in prompt
    assert VSCODE_BAD_SUMMARY in prompt
    assert "Repository identity is a publication fact" in prompt


def test_final_review_mechanically_fails_the_vscode_identity_regression(tmp_path):
    class Store:
        def enrichment(self):
            return [{
...
def test_architecture_pass_repairs_an_identity_violation_before_writing():
    ...
    class Invoker:
        ...
        def __call__(self, prompt):
            ...
            summary = (
                ...
                else "Visual Studio Code is represented at the analyzed repository snapshot."
            )
    ...
    assert not errors
    assert payload["summary"].startswith("Visual Studio Code is represented")
