...
strength. C++ is deliberately conservative (see its rules below): namespaces
make bare-name resolution noisy, so it anchors only on unambiguous type-reference
shapes and skips bare variable declarations.
"""
...
REFERENCE_LANGUAGES = frozenset({"swift", "python", "typescript", "javascript", "cpp"})
